...
import { Mail, Lock, Shield, RefreshCw, User } from 'lucide-react';
...
function AdminSignupPage() {   
  ...
  const handleSignupSubmit = async (e) => {
    ...
    try {
      // Changed to admin/signup
      const res = await axios.post(`${process.env.REACT_APP_API_URL}/api/auth/admin/signup`, { email, password, name });
  ...
  const handleVerificationSubmit = async (e) => {
    ...
    try {
      await axios.post(`${process.env.REACT_APP_API_URL}/api/auth/admin/verify`, { email: emailForVerification, code: fullOtp });
      navigate('/login', { replace: true });
    } catch (err) {
      ...
      <div className="whatsapp-card">
        {step === 1 && (
          <form onSubmit={handleSignupSubmit}>
            <h2 className="whatsapp-header">🔐 Admin Signup</h2>
            <div style={{ position: 'relative', marginBottom: '1rem' }}>
              <User style={{ position: 'absolute', left: '10px', top: '50%', transform: 'translateY(-50%)', color: '#8696a0' }} size={20} />
              <input
                type="text"
                placeholder="Name"
                value={name}
            ...
            <Link to="/login" className="whatsapp-link" style={{ display: "block", marginTop: "1rem" }}>Already have an account? Login</Link>
          </form>
        )}

        {step === 2 && (
          <div>
            <h2 className="whatsapp-header">🔐 Verify Your Email</h2>
            <p className="whatsapp-text-muted" style={{ fontWeight: 'bold', color: '#fff' }}>📨 Your registration is pending approval. Code has been routed to Super Admin. Please contact Super Admin manually to activate your gateway connection.</p>
